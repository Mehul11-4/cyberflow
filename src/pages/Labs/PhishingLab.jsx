...
import {
  ...
  XCircle, Eye, ShieldAlert, Shield,
  ...
  if (clicked) {
    return (
      <motion.div
        ...
        className="min-h-96 flex flex-col items-center justify-center text-center p-8 space-y-4"
      >
        ...
        <p className="text-gray-400 text-sm max-w-sm leading-relaxed">
          You just submitted your username and password to an attacker's server.
          In a real attack, the attacker now has full access to your account.
        </p>
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 text-left w-full max-w-sm">
          ...
          <p className="font-mono text-xs text-gray-400 mt-1">sent_to: <span className="text-red-300">{scenario.fakeLoginUrl}</span></p>
        </div>
        <p className="text-gray-500 text-xs max-w-sm">
          After stealing credentials, the attacker typically redirects you to the real website
          so you think you just mistyped your password — and never realize you were phished.
        </p>
        <button
          onClick={onBack}
    ...
    >
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 flex items-start gap-2">
        ...
        <p className="text-yellow-400 text-xs leading-relaxed">
          <strong>Educational simulation only.</strong> This fake login page shows what a victim would see after clicking the phishing link. No real data is transmitted.
        </p>
      </div>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-700 bg-gray-900">
          ...
          <div className="flex-1 flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-lg px-3 py-1">
            <AlertTriangle size={11} className="text-red-400 flex-shrink-0" />
            <span className="text-red-400 text-xs font-mono truncate">
        ...
        {/* Fake login form */}
        <div className="p-6 space-y-4" style={{ backgroundColor: '#f8f9fa' }}>
          <div className="text-center">
            <div
              className="w-10 h-10 rounded-lg mx-auto mb-3 flex items-center justify-center text-white font-bold text-sm"
              style={{ backgroundColor: scenario.fakeLoginColor }}
            >
              {scenario.fakeLoginBrand?.charAt(0)}
            </div>
            <h3 className="text-gray-800 font-bold text-lg">{scenario.fakeLoginBrand}</h3>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-lg pl-9 pr-4 py-2.5 text-sm text-gray-800 outline-none focus:border-blue-500 bg-white"
                />
              </div>
            </div>
            <div>
          ...
          <p className="text-center text-gray-400 text-xs">
            © {scenario.fakeLoginBrand} — All rights reserved
          </p>
        </div>
      </div>
...
function EmailViewer({ scenario, onViewLogin }) {
  ...
  const totalFlags = scenario.email.body.filter(b => b.suspicious).length
  ...
  function clickBodyPart(part) {
    ...
    if (!foundFlags.includes(part.id)) {
      setFoundFlags(prev => [...prev, part.id])
    }
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-sm font-bold ${c.text}`}>
            {foundFlags.length} / {scenario.indicators.length}
          </span>
        </div>
        <div className="w-full bg-gray-800 rounded-full h-2">
          <motion.div
            ...
            style={{
              width: `${score}%`,
              backgroundColor: score === 100 ? '#10b981' : score > 50 ? '#f59e0b' : '#ef4444'
            }}
          />
        </div>
        {score === 100 && (
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
        ...
        <div className="border-b border-gray-800 p-4 space-y-2">
          <div className="flex items-start gap-2">
            ...
            <span className={`text-xs font-mono font-semibold ${
              scenario.email.from.includes('-') || !scenario.email.from.split('@')[1]?.split('.').length >= 3
                ? 'text-red-400'
                : 'text-gray-300'
            }`}>
              {scenario.email.fromDisplay}
            </span>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-gray-600 text-xs w-14 flex-shrink-0">To:</span>
            <span className="text-gray-400 text-xs font-mono">{scenario.email.to}</span>
          </div>
          <div className="flex items-start gap-2">
        ...
        <div className="p-5 space-y-3">
          <p className="text-yellow-400 text-xs flex items-center gap-1 mb-4">
            <Eye size={11} /> Click on suspicious parts of the email to identify red flags
          </p>
          ...
          {scenario.email.body.map(part => (
            <div key={part.id}>
              <motion.div
                ...
                whileHover={part.suspicious ? { scale: 1.01 } : {}}
                className={`text-sm leading-relaxed rounded-lg px-3 py-2 transition-all duration-200 ${
                  part.suspicious
                    ? activeFlag?.id === part.id
                      ...
                      : 'bg-yellow-500/5 border border-yellow-500/10 text-gray-300 cursor-pointer hover:border-yellow-500/30'
                    : 'text-gray-400'
                }`}
              >
                {part.suspicious && (
                  <AlertTriangle size={11} className={`inline mr-1.5 ${
                    foundFlags.includes(part.id) ? 'text-red-400' : 'text-yellow-600'
                  }`} />
                )}
                {part.text}
          ...
          {/* Fake login link button */}
          {scenario.fakeLoginUrl && (
            <div className="mt-4 pt-4 border-t border-gray-800">
              <button
                onClick={onViewLogin}
                className="flex items-center gap-2 text-xs text-blue-400 underline hover:text-blue-300 transition-colors"
              >
                <Lock size={11} />
                See what happens if you click the phishing link →
              </button>
            </div>
          )}
        </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        >
          ...
          <ChevronRight
            size={14}
            className={`text-gray-500 transition-transform ${showChecklist ? 'rotate-90' : ''}`}
          />
        </button>
                  ...
                  >
                    ...
                    <div>
                      <p className={`text-xs font-medium transition-colors ${
                        checkedItems.includes(indicator.id) ? 'text-green-400 line-through' : 'text-gray-300'
                      }`}>
                        {indicator.label}
                      </p>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-400 mt-1">
          Analyze real phishing tactics. Click suspicious elements to identify red flags.
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className={`rounded-2xl border p-5 flex items-center gap-4 ${c.bg} ${c.border}`}
      >
        <Mail size={28} className={c.text} />
        <div>
          <h2 className={`font-bold text-lg ${c.text}`}>{scenario.title}</h2>
          <p className="text-gray-400 text-sm mt-0.5">{scenario.summary}</p>
        </div>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
        {viewingLogin ? (
          ...
          >
            <FakeLoginPage
              scenario={scenario}
              onBack={() => setViewingLogin(false)}
            />
          </motion.div>
        ) : (
          <motion.div
            key={`email-${scenarioIndex}`}
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: 20 }}
          >
            <EmailViewer
              scenario={scenario}
              onViewLogin={() => setViewingLogin(true)}
            />
          </motion.div>
        )}
      </AnimatePresence>
