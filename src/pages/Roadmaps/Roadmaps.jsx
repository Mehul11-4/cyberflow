                ...
                <div className="space-y-1.5">
                  {stage.resources.map((res, i) => (
                    <a key={i} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-gray-400 hover:text-white transition-colors group">
                      <ExternalLink size={11} className="flex-shrink-0" style={{ color }} />
...
function RoadmapCard({ roadmap, onClick, completed, delay }) {
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-bold mb-1" style={{ color }}>{roadmap.title}</h3>
      <p className="text-gray-500 text-sm mb-4 leading-relaxed">{roadmap.summary}</p>

      <div className="space-y-1.5">
        {[
          { icon: Clock,       val: roadmap.timeline   },
          { icon: TrendingUp,  val: roadmap.difficulty },
          { icon: DollarSign,  val: roadmap.salary     },
        ].map(({ icon: Icon, val }, i) => (
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t border-gray-700/30">
          <div className="flex items-center gap-2 text-sm text-gray-400"><Clock size={13} style={{ color }} /><span>{roadmap.timeline}</span></div>
          <div className="flex items-center gap-2 text-sm text-gray-400"><DollarSign size={13} style={{ color }} /><span>{roadmap.salary}</span></div>
        </div>
      </motion.div>
      ...
      >
        <h3 className="font-semibold mb-2 text-sm" style={{ color }}>About This Career</h3>
      ...
      <motion.div
        ...
        style={{ background: 'rgba(17,24,39,0.8)', border: '1px solid rgba(255,255,255,0.06)' }}
      >
        <h3 className="font-semibold mb-4 flex items-center gap-2 text-sm" style={{ color }}>
          <Award size={15} /> Certifications — In Priority Order
        </h3>
        <div className="space-y-3">
        ...
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">Step-by-step paths from beginner to job-ready</p>
            </div>
          </div>
        </motion.div>
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {roadmapsData.map((r, i) => (
            <motion.div key={r.id} variants={cardAnim}>
              <RoadmapCard roadmap={r} onClick={() => setSelected(r.id)} completed={completed.includes(r.id)} delay={i * 0.1} />
            </motion.div>
          ))}
        </motion.div>
      </div>
